    ...
    async def auth(self, http_client: cloudscraper.CloudScraper, retry=3):
        ...
        try:
            ...
            if "Too Many Requests" in str(error):
                ...
                logger.info(f"{self.session_name} | <yellow>Game server not response try again later...</yellow>")
            else:
            ...
            try:
                if check_base_url() is False:
                    ...
                    if settings.ADVANCED_ANTI_DETECTION:
                        logger.warning(
                            "<yellow>Detected index js file change. Contact me to check if it's safe to continue: https://t.me/airdropscriptzone</yellow>")
                    else:
                        logger.warning(
                            "<yellow>Detected api change! Stopped the bot for safety. Contact me here to update the bot: https://t.me/airdropscriptzone</yellow>")
